...
function NutritionProgressBar({ value, target, tone, label }: { value: number; target: number; tone: "calorie" | "carbs" | "protein" | "fat"; label: string }) {
  const percent = target > 0 ? Math.min(100, Math.round((value / target) * 100)) : 0;
  return <div className={`nutrition-progress nutrition-progress--${tone}${target > 0 ? "" : " nutrition-progress--inactive"}`}><div className="nutrition-progress__meta"><span>{target > 0 ? `${percent}% target` : "Target belum diisi"}</span><span>{label}</span></div><div className="nutrition-progress__track" role="progressbar" aria-label={`Progres ${label}`} aria-valuemin={0} aria-valuemax={target || 100} aria-valuenow={Math.min(value, target || 0)}><span style={{ width: `${percent}%` }} /></div></div>;
}
      ...
      <main className="main-content">
        ...
        {page === "health" && <section className="page page--active"><PageHeading eyebrow="CHECK-IN KESEHATAN" title="Rawat tubuh sebagai bagian dari pertumbuhanmu." description="Catat yang berguna bagimu. Pelacak ini membantu pengamatan pribadi, bukan pengganti nasihat tenaga kesehatan." />
          ...
          <section className="nutrition-section"><div className="section-title"><div><p className="eyebrow">TRACKER MAKAN & MINUM</p><h2>Kenali asupanmu, dengan netral</h2></div><span className="entry-count"><Coffee size={16} /> {todayFoodEntries.length} catatan hari ini</span></div>
            ...
            <div className="nutrition-overview"><article className="nutrition-kcal"><div><p className="eyebrow">TOTAL HARI INI</p><strong>{nutritionTotals.kcal.toLocaleString("id-ID")} <small>kcal</small></strong><p>{data.nutritionTargets.kcal > 0 ? `Target pribadi: ${data.nutritionTargets.kcal.toLocaleString("id-ID")} kcal.` : "Diinput mandiri, tanpa target atau penilaian."}</p><NutritionProgressBar value={nutritionTotals.kcal} target={data.nutritionTargets.kcal} tone="calorie" label="kalori" /></div><span><UtensilsGlyph /></span></article><div className="macro-grid"><article><span>Karbohidrat</span><strong>{nutritionTotals.carbs.toLocaleString("id-ID")}<small>g</small></strong><div><p>{nutritionProgress(nutritionTotals.carbs, data.nutritionTargets.carbs, "g")}</p><NutritionProgressBar value={nutritionTotals.carbs} target={data.nutritionTargets.carbs} tone="carbs" label="karbohidrat" /></div><i className="macro-bar macro-bar--carbs" /></article><article><span>Protein</span><strong>{nutritionTotals.protein.toLocaleString("id-ID")}<small>g</small></strong><div><p>{nutritionProgress(nutritionTotals.protein, data.nutritionTargets.protein, "g")}</p><NutritionProgressBar value={nutritionTotals.protein} target={data.nutritionTargets.protein} tone="protein" label="protein" /></div><i className="macro-bar macro-bar--protein" /></article><article><span>Lemak</span><strong>{nutritionTotals.fat.toLocaleString("id-ID")}<small>g</small></strong><div><p>{nutritionProgress(nutritionTotals.fat, data.nutritionTargets.fat, "g")}</p><NutritionProgressBar value={nutritionTotals.fat} target={data.nutritionTargets.fat} tone="fat" label="lemak" /></div><i className="macro-bar macro-bar--fat" /></article></div></div>
